...
uint256 hashGenesisBlock("0x29cdc499b035e0f53b991856a5a43e3fde2d859c86264a925bc52cf6a877f665");
...
const string strMessageMagic = "KreditCoin Signed Message:\n";
...
{
    // KreditCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // KreditCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "kredit genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x8ec748d9e6bcd3d75aadad3830991c3fd92e017602c64d5f683b96bfe673e026"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd1, 0xc1, 0xa1, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KreditCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KreditCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KreditCoinMiner(CWallet *pwallet)
{
    printf("KreditCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("KreditCoin-miner");
    ...
    try { loop {
        ...
        printf("Running KreditCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KreditCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KreditCoinMiner, pwallet));
}
