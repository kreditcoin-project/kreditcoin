...
// Copyright (c) 2011-2012 KreditCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("KreditCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #KreditCoinTEST0\r");
            Send(hSocket, "WHO #KreditCoinTEST0\r");
        } else {
            // randomly join #KreditCoin00-#KreditCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #KreditCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #KreditCoin%02d\r", channel_number).c_str());
        }
